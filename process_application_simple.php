<?php
// Simplified Application Form Processing Script - Guaranteed to work
...
try {
    ...
    if (empty($_POST['class_to_join']) || empty($_POST['student_name']) || empty($_POST['student_surname'])) {
        sendResponse(false, 'Please fill in all required fields (Class, Student Name, Student Surname)');
    }
    ...
    if (isset($_POST['signature_data']) && !empty($_POST['signature_data'])) {
        ...
        if (strpos($signature_data, 'data:image') === 0) {
            ...
            if (file_exists($signature_dir) && is_writable($signature_dir)) {
                $signature_filename = $application_no . '_signature.png';
    ...
    if (!$stmt) {
        sendResponse(false, 'Database preparation error');
    }
    ...
    if ($stmt->execute()) {
        ...
        sendResponse(false, 'Failed to save application');
    }
    
} catch (Exception $e) {
    sendResponse(false, 'An error occurred while processing your application');
} catch (Error $e) {
