...
if (!is_dir($backup_dir)) {
    if (!mkdir($backup_dir, 0755, true)) {
        $error = "Failed to create backups directory. Please check permissions.";
    }
}
...
if (!class_exists('ZipArchive')) {
    $error = "ZipArchive extension is not available. Please enable the zip extension in PHP.";
}
...
function createDatabaseBackup($filename) {
    ...
    try {
        ...
        if (!$result) {
            return "Failed to get database tables: " . $conn->error;
        }
        ...
        return "Database backup error: " . $e->getMessage();
    }
}

function createFilesBackup($filename, $include_images = true) {
    if (!class_exists('ZipArchive')) {
        return "ZipArchive extension is not available.";
    }
    
    $zip = new ZipArchive();
    
    if ($zip->open($filename, ZipArchive::CREATE) !== TRUE) {
        return "Cannot create zip file: " . $filename;
    }
    
    try {
        // Add admin files (excluding backups directory)
        if (is_dir('.')) {
            addDirectoryToZip($zip, '.', 'admin/', ['backups']);
        }
        ...
        if (is_dir('../scss/')) {
            addDirectoryToZip($zip, '../scss/', 'scss/');
        }
        ...
        if (is_dir('../uploads/')) {
            addDirectoryToZip($zip, '../uploads/', 'uploads/');
        }
        ...
        $zip->close();
        return "Files backup error: " . $e->getMessage();
    }
}
...
function createFullBackup($filename, $backup_name, $include_images = true) {
    ...
    try {
        ...
        $zip->close();
        return "Full backup error: " . $e->getMessage();
    }
}
...
function addDirectoryToZip($zip, $source_dir, $zip_dir, $exclude = []) {
    ...
    try {
        ...
        // Continue even if some files can't be added
    }
}
        ...
        foreach ($statements as $statement) {
            ...
            if (!empty($statement) && !preg_match('/^--/', $statement)) {
                if (!$conn->query($statement)) {
                    return "SQL Error: " . $conn->error . " in statement: " . substr($statement, 0, 100) . "...";
                }
            }
        }
